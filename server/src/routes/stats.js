...
router.get('/', requireAdmin, async (req, res) => {
  ...
  // подменить параметры и заглянуть в чужую школу. Номер школы уникален
  // внутри района/города, а не внутри всей области, поэтому город обязателен
  // для точного совпадения (город теперь выбирается из списка, а не текстом,
  // чтобы опечатки не ломали совпадение).
  const where = {
